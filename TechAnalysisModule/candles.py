...
def findCandleFormations(O,H,L,C,trend):
    ...
    if trend=='rising':
        ...
        result.append(findIsland(O,H,L,C,'rising'))
        result.append(findEveningStar(O,H,L,C))                
    else:
        result.append(findBear3(O,H,L,C))
        result.append(findIsland(O,H,L,C,'falling'))
...
def findEveningStar(O,H,L,C):
    ...
    for i in range(len(O)-1, 2, -1):
        ...
        body3=(C[i]-O[i])/O[i]
        if body1 < LONG_BODY or body2 > SHORT_BODY or body3 > -LONG_BODY:
            continue
        if O[i-1]<=C[i-2] or O[i]>=C[i-1]:
            continue
