...
class TabA(QtGui.QWidget):
    ...
    def paintChart(self):
		...
		print dataParser.STOCK_LIST[index][2]
    ...
    def paint2Chart(self):
		...
		if dataParser.INDEX_LIST[index][2] == 'Yahoo':
            		self.finObj = dataParser.createWithArchivesFromYahoo(dataParser.INDEX_LIST[index][1],
                                                                     dataParser.INDEX_LIST[index][0],'index',
                                                                     dataParser.INDEX_LIST[index][3],step)
		else:
			self.finObj = dataParser.createWithArchivesFromStooq(dataParser.INDEX_LIST[index][1],
                                                                     dataParser.INDEX_LIST[index][0],'index',
                                                                     dataParser.INDEX_LIST[index][3],step)
	    ...
	    if dataParser.STOCK_LIST[index][2] == 'Yahoo':
           	 self.finObj = dataParser.createWithArchivesFromYahoo(dataParser.STOCK_LIST[index][1],
                                                                     dataParser.STOCK_LIST[index][0],'stock',
                                                                     dataParser.STOCK_LIST[index][3],step)
	    else:
                 self.finObj = dataParser.createWithArchivesFromStooq(dataParser.STOCK_LIST[index][1],
                                                                     dataParser.STOCK_LIST[index][0],'stock',
                                                                     dataParser.STOCK_LIST[index][3],step)
	    self.currentChart = self.stockListView.currentIndex().data(QtCore.Qt.DisplayRole).toString()

        if self.listName == "forex":
            self.finObj = dataParser.createWithArchivesFromStooq(dataParser.FOREX_LIST[index][1],
                                                                     dataParser.FOREX_LIST[index][0],'forex',
                                                                     dataParser.FOREX_LIST[index][3],step)
